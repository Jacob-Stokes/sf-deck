...
//
// Reconciliation keeps a routine unchanged org-list refetch inexpensive:
// valid tokens survive, while vanished or genuinely stale aliases
// re-bootstrap on demand.
...
//
// Used by CLI-fallback paths (e.g. RunListView's `sf api request rest`
// branch) that build /services/data/v<N>/ paths by hand. Centralising the
// lookup keeps version updates in one place.
