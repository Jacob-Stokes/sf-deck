...
// TestGroupPrefersBrowserSessionOverAPI covers the case where a
...
func TestGroupPrefersBrowserSessionOverAPI(t *testing.T) {
	...
	p := rows[0]
	if p.SourceIP != "198.51.100.42" {
		t.Errorf("representative IP = %q, want the documentation-range browser IP 198.51.100.42 (not the API session)", p.SourceIP)
	}
...
// whose IP resolves to a region but not a city ("South Holland, NL") must
...
func TestFormatGeo(t *testing.T) {
	...
	cases := []struct {
		...
		{geo("Rotterdam", "", "NL"), "Rotterdam, NL"},
		{geo("Rotterdam", "South Holland", "NL"), "Rotterdam, NL"}, // City wins over subdivision
		{geo("", "South Holland", "NL"), "South Holland, NL"},      // subdivision fallback (the bug)
		{geo("", "", "NL"), "NL"},                                  // neither → bare country
		{geo("Rotterdam", "", ""), "Rotterdam"},
		{geo("", "South Holland", ""), "South Holland"},
		{geo("", "", ""), ""},
...
func TestSessionRowLocation(t *testing.T) {
	cases := []struct {
		...
		{SessionRow{City: "Rotterdam", CountryISO: "NL"}, "Rotterdam, NL"},
		{SessionRow{Subdivision: "South Holland", CountryISO: "NL"}, "South Holland, NL"},
		{SessionRow{CountryISO: "NL"}, "NL"},
		{SessionRow{}, ""},
	}
	for _, c := range cases {
...
func TestCleanSourceIP(t *testing.T) {
	...
	if got := cleanSourceIP("198.51.100.42"); got != "198.51.100.42" {
