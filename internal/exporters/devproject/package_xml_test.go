...
func TestMetadataMember_AllSupportedKinds(t *testing.T) {
	cases := []struct {
		...
		{"flow blank-name devname-ref", devproject.Item{Kind: devproject.KindFlow, Ref: "Carrier_Onboarding"}, "Flow", "Carrier_Onboarding", true},
		// Blank-name flow whose Ref IS a DefinitionId → cannot emit (a
		// 300-id is not a valid member); honestly dropped, not guessed.
		{"flow blank-name defid-ref", devproject.Item{Kind: devproject.KindFlow, Ref: "300DM00000TEST1AAA"}, "", "", false},
