...
func TestRecord_OrgNotFound(t *testing.T) {
	a := newRecordTestApp()
	code, got := runRecordCLI(t, a, "--json", "record", "get",
		"--org", "missing", "--id", "001DM00000TEST3AAA")
...
func TestWriteRecordErr_MissingRecordMapsNotFound(t *testing.T) {
	...
	code := writeRecordErr("record.get", "dev@example.com", "Account", "001DM00000MISSING",
		errSimple("no Account with Id 001DM00000MISSING"), &stdout, headless.JSONMode)
	if code != headless.ExitNotFound {
	...
	if details["object"] != "Account" || details["id"] != "001DM00000MISSING" {
