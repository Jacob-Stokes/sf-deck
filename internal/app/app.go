...
//     order, returning an *App ready to use across all entry points.
...
func Open(opts OpenOptions) (*App, error) {
	...
	// Harden the base ~/.sf-deck directory to owner-only before any entry
	// point opens a database inside it. TUI and headless commands share
	// the same filesystem-permission floor. Best-effort: a chmod failure
	...
	// Push the [ui.api] settings (timeouts, poll intervals, forced API
	// version) into the sf package so every entry point — TUI, headless,
	// and demo — respects the user's tuning.
	...
	// Enforce the process-memory boundary for record and list-view data
	// by clearing their reserved persistent-cache prefixes at startup.
