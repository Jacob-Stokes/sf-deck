...
func TestApplyReconcile_MergeDuplicate(t *testing.T) {
	s := openTestStore(t)
	s.CreateDevProject(DevProject{ID: "p", Name: "P", CreatedAt: time.Now(), TouchedAt: time.Now()})
	seedItem(t, s, "p", "alice", KindFlow, "Carrier_Onboarding", "")       // import row
	seedItem(t, s, "p", "alice", KindFlow, "300XYZ", "Carrier_Onboarding") // collect row
