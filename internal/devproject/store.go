...
// migrateRecordItemRefs normalises compatible bare "<Id>" KindRecord refs
// (sObject in the type column) to the canonical "<sObject>:<Id>" shape used
// by tag/project lookups. Idempotent: normalised rows contain ':' and are
// skipped on later opens.
//
...
// possible if the same record is present in both shapes); the
