...
func TestDispatchOtherOrgPreviewAddsStripRow(t *testing.T) {
	st := makeSettings(t, []settings.ChipConfig{
		{
			ID:     "northwind-shipments",
			Label:  "Northwind Shipments",
	...
	(&m).dispatchChipManagerAction(domainFlows, "*", "otherpreview:northwind-shipments")

	if got := m.chipPreviewsFor(domainFlows, "*"); len(got) != 1 || got[0].Chip.ID != "northwind-shipments" {
		t.Fatalf("preview dispatch did not store preview: %+v", got)
	}
	rows := m.stripRows(domainFlows, "*")
	var found *chipRow
	for i := range rows {
		if rows[i].ID == "northwind-shipments" {
			found = &rows[i]
			break
		}
	}
...
func TestApplyChipScopeChosenOtherOrgPersistsGlobalShare(t *testing.T) {
	...
	if rows := reg.ChipsFor("*"); len(rows) != 1 || rows[0].ID != "northwind-shipments" {
