...
func TestOrgStartupPinSurvivesInitialCacheMiss(t *testing.T) {
	st := &settings.Settings{Orgs: map[string]settings.OrgConfig{
		"staging@example.com": {Default: true},
	}}
	...
	liveOrgs := []sf.Org{
		...
		{Username: "staging@example.com", Alias: "staging-sandbox"},
	}
	m.orgsRes = Resource[[]sf.Org]{
		Scope: "global", Key: "orgs",
	...
	m = next.(Model)
	if got := m.orgs[m.selected].Username; got != "staging@example.com" {
		t.Fatalf("selected org = %q, want pinned staging@example.com", got)
	}
	if m.selectedUsername != "staging@example.com" {
		t.Fatalf("selectedUsername = %q, want staging@example.com", m.selectedUsername)
	}
	if !m.pinnedDefaultRestored {
