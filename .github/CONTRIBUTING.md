...
Never commit data copied from a real Salesforce org. This includes org
aliases and usernames, people or company names, IP addresses, record IDs,
tenant URLs, metadata API names, labels, counts, screenshots, logs, and
comments describing where behaviour was verified.

Use the fictional Northwind vocabulary already present in `--demo`. Use
`example.com` / `.example` for domains, RFC 5737 documentation ranges for
IP addresses, and conspicuously synthetic Salesforce IDs containing `DM`.
Sanitise regression fixtures before committing them, even when the source
org is a sandbox you own.
...
- **Never record the sandbox's identity or data in the repository.** PRs,
  fixtures, screenshots, logs, comments, and commit messages must use
  sanitised Northwind examples only.
