...
all route through this. Bypassing the cache in any new resolver = perf
...
This catches UI regressions at unit-test time before a user switches tabs.
...
Centralising the strip wiring ensures every branch renders the same subtab
navigation and budget handling.
