...
> to generate the sf fetcher + every boilerplate snippet. The plumbing is
> owned by **one `registerListResource(...)` entry** (see
...
> global-refresh enumeration, without a hand-written `case "x_v1":` in
> update.go or a `SyncXList()` line. The drift tests
...
Single-mode tabs skip this step. Tabs with multiple subtabs (Reports, Apex, Components, SOQL, Meta) **must** route through `dispatchSubtab` — it draws the subtab strip exactly once and routes to the active branch.
...
**Reality:** `tabRegistry()` is the source of truth. If your tab isn't there, it falls outside the standard dispatch contract.
...
**Symptom:** "I'll just render the strip at the top of my main renderer and prepend it in each branch." A missed branch or callsite can make the subtab pills inconsistent.
...
**Rule:** if your tab has subtabs, route through `dispatchSubtab`. Branch renderers that take `subStrip` as a parameter should be migrated to the shared dispatcher.
...
The coordination requirement is that **mutations invalidate every org's
snapshot**, otherwise switching orgs after a save can show stale data. The
pattern is `invalidateSOQLSaved()`:
...
Every mutation handler calls it; review shared-store changes against this
invariant.
...
2. **Invalidation paths start multiplying.** When keeping mutation paths aligned requires repeated manual invalidation, make the invariant structural.

Don't pre-emptively migrate just for elegance. The current pattern works; the planned one becomes worthwhile when either trigger applies.
