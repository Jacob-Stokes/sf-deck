...
Status: maintained recording brief.
...
- Use only the fictional Northwind demo described in this document.
- Inspect every rendered GIF and poster at full size before publishing.
- Confirm that filenames, terminal titles, visible rows, and shell prompts use
  only the approved fictional fixtures.
...
6. Test desktop, mobile, reduced-motion, keyboard navigation, and broken-image
   behavior locally.
7. Run a final content and security review before publishing.
