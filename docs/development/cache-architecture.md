...
- **List views** (`listviews:`, `listview:`) — per-user sharing and live
  results make these process-local. Startup clears the reserved prefixes to
  enforce the disk boundary.
...
### Disk-boundary enforcement

`cmd/sf-deck/main.go:69` clears prefixes reserved for process-memory data on
every process start:
...
The corresponding Resources are `NoCache:true`, so the prefixes remain empty
during normal operation. Clearing them at startup makes the storage boundary
self-enforcing.
...
stays `fetchedAt=zero` forever. The route table is the loop-closer.
...
| Startup boundary enforcement | `DeleteKeyPrefix("records:" / "listviews:" / "listview:" / "recently_viewed")` | process-memory-only prefixes |
